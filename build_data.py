...
def canon_key(base):
    """Punctuation-insensitive grouping key: two titles that differ only by
    separators (':' vs ' - ', commas, extra spaces, hyphens) collapse to the
    same movie. 'Nagabandham: The Secret Treasure' and 'Nagabandham - The
    Secret Treasure' both -> 'nagabandham the secret treasure'."""
    t = canonical_title(base).casefold()
    t = re.sub(r"[^0-9a-z]+", " ", t).strip()      # colons/dashes/commas -> space
    return t or canonical_title(base).casefold()
...
def build_date(mode, date, src_dir, out_dir, posters, metadata, pub_dir=None):
    ...
    for ck, mrows in grouped.items():
        ...
        index.append({
            ...
            "releaseDate": (movie["meta"]["releaseDate"] if movie.get("meta") else None),
        })
